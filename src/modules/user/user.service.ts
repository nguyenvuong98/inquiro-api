...
import {
  UserAddPermissionInput,
  UserDto,
  UserRegiterRequestDdto,
} from './user.dto';
...
export class UserService {
  ...
  async registerUser(userRegister: UserRegiterRequestDdto): Promise<UserDto> {
    ...
    const hash = await bcrypt.hash(
      ...
      this.configService.get('saltOrRound'),
    );
    ...
    const response = _.pick(
      { id: user._id.toString(), ...user, accessToken },
      USER_PICK_KEYS,
    );
    ...
    this.cacheManager.set(
      ...
      parseInt(this.configService.get('jwt.jwtExpireTime').toString()),
    );
  ...
  async createToken(user: any) {
    ...
    const accessToken = await this.jwtService.signAsync(payload, {
      secret: this.configService.get('jwt.jwtSecretKey'),
    });
  ...
  async addPermission(input: UserAddPermissionInput) {
    ...
    const updatePermission = await this.userRepository.updateOne(
      { email: input.email },
      queryUpdate,
    );
